export const minNumber = (value: any, values: any, form: any) => { // is the number less than or equal to the max?
    ...
    if (form && form.fieldData && form.fieldData[thisFieldName]){
        ...
        if (thisField.typeSpecific && thisField.typeSpecific.min > -1){

            const minNumber: number = parseInt(thisField.typeSpecific.min, 10);
            const inputValue: number = parseInt(value, 10);

            if (inputValue < minNumber) {
                return 'Value less than ' + minNumber;
            }
